...
\begin{equation}
	\begin{aligned}
		...
		&= \frac{1}{N} \sum_{i=1}^N \Var \left[ \frac{f(X_i)}{p(X_i)} \right] \\
		&= \Var \left[ \frac{f(X_1)}{p(X_1)} \right] \\
		&= \E \left[ \frac{f(X_1)}{p(X_1)} \right]^2 - I^2 \\
		&= \int_\Omega \frac{f(x)^2}{p(x)} \diff x - I^2.
	\end{aligned}
\end{equation}

从这里可以发现，当被积函数 $f(x)$ 在某些区域值较大时，如果采样分布 $p(x)$ 在这些区域的概率较小，则会导致方差增大。因此，选择合适的采样分布对于降低方差至关重要。
...
\end{equation}
由KKT条件中的可行性，即归一化条件，可以得到最优的概率分布为：
\begin{equation}
