...
在~\eqref{eq:mc-variance}的分析中，已经知道，$f(x)$在某些区域值较大时，如果采样分布$p(x)$在这些区域的概率较小，则会导致方差增大。
...
\end{figure}
该图是对Veach论文\cite{Veach1995}中MIS效果的复现实验。
场景的最上方是$5$个大小，颜色互异的球状光源，
下方是四面模糊度不同的长方形镜面（从上倒下模糊度递增）。
相机可以通过镜面反射看到光源（下方看到的$20$个亮斑），也可以直接看到光源（最上面看到的$5$个光源）。
